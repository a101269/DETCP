...
def run_dist_n_evaluation(prompts):
    ...
    for t in texts:
        ...
        two_gram.extend([' '.join(tokens[i : i + 2]) for i in range(len(tokens) - 1)])  # bi-gram
    dist_1 = len(set(one_gram)) / len(one_gram)  # 词汇多样性
...
eval_fp = osp.join(f'results1', fn + '.jsonl')
evals = list(jsl.open(eval_fp, 'r'))
eval_fp = osp.join(f'results2', fn + '.jsonl')
evals += list(jsl.open(eval_fp, 'r'))
...
print(
    "\t".join(
        map(
            str,
            [
                avg_max_toxicity,  # 最大毒性平均数,(Exp.Max.Tox.
                std_max_toxicity,  # 标准差
                toxic_probs,  # 有毒内容毒性平均值, Tox. Prob
                avg_ppl,
                dist_1,  # 多样性
                dist_2,
            ],
